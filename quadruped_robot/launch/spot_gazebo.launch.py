...
def generate_launch_description():
    ...
    gz_pkg_share = launch_ros.substitutions.FindPackageShare(package="quadruped_robot").find(
        "quadruped_robot"
    )
    links_config = os.path.join(gz_pkg_share, "config/spot_links.yaml")
    ...
    declare_gazebo_world = DeclareLaunchArgument(
        "world", default_value=os.path.join(gz_pkg_share, "worlds/env.world")
    )
    ...
    default_model_path = os.path.join(gz_pkg_share, "urdf/spot.urdf.xacro")
    ...
    ).find("quadruped_robot"), "config/gazebo.yaml")
    ...
    contact_sensor = Node(
        ...
        output="screen",
        parameters=[{"use_sim_time": LaunchConfiguration("use_sim_time")},LaunchConfiguration('links_map_path')],
